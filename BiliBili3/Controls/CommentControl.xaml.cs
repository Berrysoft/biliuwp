    ...
    public sealed partial class CommentControl : UserControl
    {
        ...
        public async void LoadComment_JustHot()
        {
            try
            {
                ...
                string url = "http://api.bilibili.com/x/reply?type=1&sort=2&oid=" + _aid + "&pn=" + pageNum + "&ps=5&platform=wp&appkey=" + ApiHelper.AndroidKey.Appkey + "&access_key=" + ApiHelper.AccessKey + "&rnd" + new Random().Next(1, 9999);
        ...
        public async void LoadComment()
        {
            try
            {
                ...
                _Loading = true;
                pr_load.Visibility = Visibility.Visible;
                string url = "http://api.bilibili.com/x/reply?type=1&sort=0&oid=" + _aid + "&pn=" + pageNum + "&ps=20&platform=wp&appkey=" + ApiHelper.AndroidKey.Appkey + "&access_key=" + ApiHelper.AccessKey + "&rnd" + new Random().Next(1, 9999);
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
                NewCommentModel model = JsonConvert.DeserializeObject<NewCommentModel>(results);
                if (model.code == 0)
        ...
        public async void LoadComment_Reply(string aid)
        {
            try
            {
                ...
                string url = "http://api.bilibili.com/x/reply/reply?oid=" + aid + "&pn=" + pageNum_Reply + "&ps=20&root=" + rpid + "&type=1&access_key=" + ApiHelper.AccessKey + "&appkey=" + ApiHelper.AndroidKey.Appkey + "&rnd" + new Random().Next(1, 9999); ;
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
                NewCommentModel model = JsonConvert.DeserializeObject<NewCommentModel>(results);
                if (model.code == 0)
        ...
        public async void LoadHotComment(string aid)
        {
            try
            {
                ...
                string url = "http://api.bilibili.com/x/reply?type=1&sort=1&oid=" + aid + "&pn=1&ps=20&platform=wp&appkey=" + ApiHelper.AndroidKey.Appkey + "&access_key=" + ApiHelper.AccessKey + "&rnd" + new Random().Next(1, 9999); ;
                url += "&sign=" + ApiHelper.GetSign(url);
                string results = await WebClientClass.GetResults(new Uri(url));
                NewCommentModel model = JsonConvert.DeserializeObject<NewCommentModel>(results);
                if (model.data.replies.Count != 0)
        ...
        private async void btn_Send_Click(object sender, RoutedEventArgs e)
        {
            ...
            if (UserManage.IsLogin())
            {
                try
                {
                    string uri = string.Format("http://api.bilibili.com/x/reply/add?_device=wp&build={2}&platform=wp&appkey={0}&access_key={1}", ApiHelper.AndroidKey.Appkey, ApiHelper.AccessKey, ApiHelper.build);
