...
namespace BiliBili3.Modules
{
    public class Home : IModules
    {
        ...
        public async Task<ReturnModel<ObservableCollection<HomeDataModel>>> GetHome(string idx = "0")
        {
            try
            {
                string url = $"https://app.bilibili.com/x/feed/index?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&flush=0&idx={idx}&login_event=2&mobi_app=android&network=wifi&open_event=&platform=android&pull={(idx == "0").ToString().ToLower()}&qn=32&style=2&ts={ApiHelper.TimeStamp}";
        ...
        public async Task<ReturnModel<object[]>> GetHot(string last_param = "", string idx = "0")
        {
            try
            {
                string url = $"https://app.bilibili.com/x/v2/show/popular/index?appkey={ApiHelper.AndroidKey.Appkey }&build={ApiHelper.build}&fnval=16&fnver=0&force_host=0&fourk=1&idx={idx}&last_param={last_param}&mobi_app=android&platform=android&qn=32&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
                results = results.Replace("goto", "_goto");
        ...
        public async Task<ReturnModel<ObservableCollection<TabModel>>> GetTab()
        {
            try
            {
                string url = $"https://app.bilibili.com/x/feed/index/tab?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
        ...
        public async Task<ReturnModel<TabDataModel>> GetTabData(int tab_id)
        {
            try
            {
                string url = $"https://app.bilibili.com/x/feed/index/tab?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&id={tab_id}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
                results = results.Replace("goto", "_goto");
                var model = results.ToDynamicJObject();
        ...
        public async Task<ReturnModel> UnLike(string _goto, string id)
        {
            try
            {
                string url = $"https://app.bilibili.com/x/feed/dislike?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&goto={_goto}&id={id}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
                var model = JsonConvert.DeserializeObject<HomeFeedModel>(results);
                if (model.code == 0)
        ...
        public async Task<ReturnModel> UnLikeNeedReason(string _goto, string id, string mid, int reason_id, string rid, string tag_id)
        {
            try
            {
                string url = $"https://app.bilibili.com/x/feed/dislike?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&goto={_goto}&id={id}&mid={mid}&mobi_app=android&platform=android&reason_id={reason_id}&rid={rid}&tag_id={tag_id}&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var results = await WebClientClass.GetResults(new Uri(url));
                var model = JsonConvert.DeserializeObject<HomeFeedModel>(results);
                if (model.code == 0)
