    ...
    public class Account : IModules
    {
        ...
        public async Task<ReturnModel> Follow(int uid)
        {
            try
            {
                ...
                string data = $"access_key={ApiHelper.AccessKey}&act=1&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&fid={uid}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}&re_src=31";
        ...
        public async Task<ReturnModel> UnFollow(int uid)
        {
            ...
            {
                string url = "https://api.bilibili.com/x/relation/modify";
                string data = $"access_key={ApiHelper.AccessKey}&act=2&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&fid={uid}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}&re_src=31";
                data += "&sign=" + ApiHelper.GetSign(data);
                string result = await WebClientClass.PostResults(new Uri(url), data);
                JObject jb = JObject.Parse(result);
                if ((int)jb["code"] == 0)
        ...
        private static async Task<string> EncryptedPassword(string passWord)
        {
            ...
            try
            {
                ...
                string content = $"appkey={ApiHelper.AndroidKey.Appkey}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
        ...
        public async Task<LoginCallbackModel> LoginV3(string username, string password)
        {
            try
            {
                ...
                string data = $"username={username}&password={pwd}&gee_type=10&appkey={ApiHelper.AndroidKey.Appkey}&mobi_app=android_comic&platform=android&ts={ApiHelper.TimeStamp}";
                data += "&sign=" + ApiHelper.GetSign(data);
        ...
        public async Task<LoginCallbackModel> LoginV2(string username, string password, string captcha = "")
        {
            try
            {
                ...
                string data = $"appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&mobi_app=android&password={Uri.EscapeDataString(await EncryptedPassword(password))}&platform=android&ts={ApiHelper.TimeStamp}&username={Uri.EscapeDataString(username)}";
                if (data != "")
                {
        ...
        public async Task SSO(string access_key)
        {
            try
            {
                ...
                var url = $"https://passport.bilibili.com/api/login/sso?access_key={access_key}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&gourl=https%3A%2F%2Faccount.bilibili.com%2Faccount%2Fhome&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
        ...
        public async Task<ReturnModel> RefreshToken(string access_key, string refresh_token)
        {
            try
            {
                ...
                var data = $"access_token={access_key}&refresh_token={refresh_token}&appkey={ApiHelper.AndroidKey.Appkey}&ts={ApiHelper.TimeStamp}";
                data += "&sign=" + ApiHelper.GetSign(data);
        ...
        public async Task<ReturnModel> CheckLoginState(string access_key)
        {
            try
            {
                var url = $"https://passport.bilibili.com/api/oauth2/info?access_token={access_key}&appkey={ApiHelper.AndroidKey.Appkey}&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
                var content = await WebClientClass.GetResults(new Uri(url));
                var obj = JObject.Parse(content);
                if (obj["code"].ToInt32() == 0)
        ...
        public async Task<ReturnModel<MyInfoModel>> GetMyInfo()
        {
            try
            {
                var url = $"https://app.bilibili.com/x/v2/account/myinfo?access_key={ApiHelper.AccessKey}&appkey={ApiHelper.AndroidKey.Appkey}&build={ApiHelper.build}&mobi_app=android&platform=android&ts={ApiHelper.TimeStamp}";
                url += "&sign=" + ApiHelper.GetSign(url);
        ...
        public static async Task<string> AuthBiliPlus()
        {
            try
            {
                ...
                var url = new Uri($"https://www.biliplus.com/login?act=savekey&mid={SettingHelper.UserID}&access_key={ApiHelper.AccessKey}&expire=");
